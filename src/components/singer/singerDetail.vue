<template>
  <div class="m-singerDetail-wrap">
    <!-- 页头 -->
    <div class="m-page-header">
      <el-page-header @back="goBack" content="歌手详情">
      </el-page-header>
    </div>
    <!-- 歌手信息 -->
    <div class="m-top-wrap">
      ...
      <div class="m-right">
        ...
        <p class="m-num">
          关注数: <span>{{singerinfo.fansCount | playCountFormat}}</span>
        </p>
        <p class="m-num">
          单曲数: <span style="margin-right:10px">{{singerDetail.musicCount}}</span>
          专辑数: <span style="margin-right:10px">{{singerDetail.albumCount}}</span>
          MV数: <span>{{singerDetail.mvCount}}</span>
        </p>
        <p class="m-desc">简介: <span>{{singerDetail.desc}}</span></p>
      </div>
    </div>
    <!-- 详细列表 -->
    <div class="m-list-wrap">
      <el-table :data="playlist" style="width: 100%;" size="mini">
...
export default {
  ...
  watch: {
    singerInfo (newV) {
      this.singerinfo = newV
      this.playlist = []
      this.getMusicOfSingerById()
      this.getSingerDetail()
    }
  },
  methods: {
    // 获取歌手热门50首音乐
    async getMusicOfSingerById () {
      const param = {
        id: this.singerinfo.id
      }
      const res = await api.getMusicOfSingerById(param)
